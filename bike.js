...
document.getElementById("bikeForm").addEventListener("submit", function(event) {
    ...
    if (name && email && route && date) {
        ...
        document.getElementById("formFeedback").textContent = `Thank you, ${name}! You've successfully joined the bike pool for the route: ${route} on ${date}.`;
        document.getElementById("bikeForm").reset(); // Reset the form after submission
    } else {
